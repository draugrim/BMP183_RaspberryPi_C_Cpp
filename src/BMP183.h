...
public:
    BMP183();
    BMP183(std::string devspi, unsigned char spiMode, unsigned int spiSpeed, unsigned char spibitsPerWord);
    ~BMP183();

    bool begin(bmp183_mode_t mode = BMP183_MODE_STANDARD);
...
public:
    ...
    static void sleep(unsigned long secs) {QThread::usleep(secs);}
};
